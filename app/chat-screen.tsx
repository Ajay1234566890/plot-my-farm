...
export default function ChatScreen() {
  ...
  useEffect(() => {
    if (user?.id) {
      if (currentChatId) {
        ...
        // Initialize chat if chatId is missing
        initializeChat();
      }
      fetchPhoneNumber();
    }
  ...
  const initializeChat = async () => {
    try {
      // 1. Ensure updated profile for Current User (Farmer)
      if (user) {
        await upsertUser({
          id: user.id,
          name: (user as any).user_metadata?.full_name || 'Farmer',
          role: 'farmer',
      ...
      // 2. Ensure profile for Target User (Buyer)
      await upsertUser({
        id: userId,
        name: userName,
        role: 'buyer',
      ...
      // 3. Create or get chat with Farmer (user.id) and Buyer (userId)
      const { data, error } = await createOrGetChat(user!.id, userId);
              ...
              return (
                ...
                >
                  <View
                    ...
                    style={{
                      backgroundColor: isMe ? '#7C8B3A' : '#FFFFFF',
                      shadowColor: '#000',
      ...
      <View className="px-6 pb-6">
        ...
        >
          ...
          <TouchableOpacity
            ...
            style={{ backgroundColor: sending ? '#9CA3AF' : '#7C8B3A' }}
